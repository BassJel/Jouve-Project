...
import com.doculibre.constellio.solr.handler.component.ConstellioElevationComponent.ElevationObj.ElevationInclude;
 ...
 * @since solr 1.3
 */
public class ConstellioElevationComponent extends SearchComponent implements SolrCoreAware {
    private static Logger log = LoggerFactory.getLogger(ConstellioElevationComponent.class);
...
//    private Analyzer analyzer = null;
    ...
    class ElevationObj {
        final String elevateQueryId;
        final String analyzed;
        final BooleanClause[] exclude;
        // final BooleanQuery include;
        // final Map<String, Integer> priority;
        // private List<String> elevateClick = null;
        private List<String> elevate;
        private String collectionName;

        ElevationObj(String collectionName, String qstr, List<String> elevate, List<String> exclude)
            throws IOException {
            this.collectionName = collectionName;
            this.elevateQueryId = qstr;
//            this.analyzed = getAnalyzedQuery(this.text);
            this.analyzed = this.elevateQueryId;
        ...
        public ElevationInclude getElevationInclude() {
            return new ElevationInclude(elevateQueryId);
        }

        // updatePriority(List<String> elevateClick){
        // this.include = new BooleanQuery();
        // this.include.setBoost(0);
        // this.priority = new HashMap<String, Integer>();
        // int max = elevate.size() + elevateClick.size() + 5;
        ...
        class ElevationInclude {

        	final String elevateQueryId;
            final BooleanQuery include;
            final Map<String, Integer> priority;
            final List<String> elevateClick;
            // use singletons so hashCode/equals on Sort will just work
            final FieldComparatorSource comparatorSource;

            public ElevationInclude(String elevateQueryId) {
            	this.elevateQueryId = elevateQueryId;
                this.elevateClick = initElevateClick();
                this.include = new BooleanQuery();
                include.setBoost(0);
                this.priority = new HashMap<String, Integer>();
            ...
            private List<String> initElevateClick() {
                ...
                if (resultsRelevance != null) {
                    if (resultsRelevance.isActive()) {
                    	ElevateServices elevateServices = ConstellioSpringUtils.getElevateServices();
                    	SimpleSearch simpleSearch = elevateServices.toSimpleSearch(elevateQueryId);
                    	
                        int minClicks = resultsRelevance.getMinClicks();
                        // a cause de StatsCompiler.computeClickUrl : le nombre de click commence à 0
                        minClicks--;
                        int maxResults = resultsRelevance.getMaxResults();

                        Date now = new Date();
                        Date startDate = DateUtils.addYears(now, -1); // Default
                        startDate = ConstellioDateUtils.getBeginningOfDay(startDate);
                        
                        Date endDate = ConstellioDateUtils.getEndOfDay(now);
                        StatsServices statsServices = ConstellioSpringUtils.getStatsServices();
                    	SolrDocumentList mostClickedDocuments = statsServices.getMostClickedDocuments(simpleSearch, startDate, endDate, true, minClicks, maxResults);
                    	
                        for (SolrDocument currentDoc : mostClickedDocuments) {
                            returnList.add((String) currentDoc.getFieldValue(StatsConstants.INDEX_FIELD_RECORD_URL));
                        }
                    }
                }
                return returnList;
    ...
    public void inform(SolrCore core) {
        ...
        if (a != null) {
            ...
            if (ft == null) {
                throw new SolrException(SolrException.ErrorCode.SERVER_ERROR, "Unknown FieldType: '" + a
                    + "' used in ConstellioElevationComponent");
            }
//            analyzer = ft.getQueryAnalyzer();
        ...
        if (sf == null || !(sf.getType() instanceof StrField)) {
            throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                "ConstellioElevationComponent requires the schema to have a uniqueKeyField implemented using StrField");
        }
        ...
        try {
            synchronized (elevationCache) {
                ...
                if (f == null) {
                    throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                        "ConstellioElevationComponent must specify argument: '" + CONFIG_FILE
                            + "' -- path to elevate.xml");
                }
                ...
                if (fC.exists() == fD.exists()) {
                    throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                        "ConstellioElevationComponent missing config file: '" + f + "\n" + "either: "
                            + fC.getAbsolutePath() + " or " + fD.getAbsolutePath()
                            + " must exist, but not both.");
                }
                if (fC.exists()) {
                    log.info("Loading QueryElevation from: " + fC.getAbsolutePath());
                    Config cfg = new Config(core.getResourceLoader(), f);
                    elevationCache.put(null, loadElevationMap(cfg, core.getName()));
                } else {
            ...
            throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                "Error initializing ConstellioElevationComponent.", ex);
        }
    }
    ...
    Map<String, ElevationObj> getElevationMap(IndexReader reader, SolrCore core) throws Exception {
        synchronized (elevationCache) {
            ...
            if (map == null) {
                ...
                map = loadElevationMap(cfg, collectionName);
    ...
    private Map<String, ElevationObj> loadElevationMap(Config cfg, String collectionName) throws IOException {
        ...
        for (int i = 0; i < nodes.getLength(); i++) {
            ...
            ElevationObj elev = new ElevationObj(collectionName, qstr, include, exclude);
            if (map.containsKey(elev.analyzed)) {
                throw new SolrException(SolrException.ErrorCode.SERVER_ERROR,
                    "Boosting query defined twice for query: '" + elev.elevateQueryId + "' (" + elev.analyzed + "')");
            }
     ...
     * @throws IOException
     */
    void setTopQueryResults(IndexReader reader, String query, String collectionName, String[] ids, String[] ex)
        throws IOException {
        if (ids == null) {
            ids = new String[0];
        }
        ...
        ElevationObj obj = new ElevationObj(collectionName, query, Arrays.asList(ids), Arrays.asList(ex));
...
//    }
    ...
    public void prepare(ResponseBuilder rb) throws IOException {
        ...
        if (collectionName == null) {
            log.warn("Collection name is null");
            return;
        }
        RecordCollectionServices recordCollectionServices = ConstellioSpringUtils
            .getRecordCollectionServices();
        ...
        RecordCollection collection = recordCollectionServices.get(collectionName);
        if (collection == null) {
            log.error("Collection is null for name " + collectionName);
            return;
        }
        ...
        String simpleSearchStr = params.get(ConstellioSolrQueryParams.SIMPLE_SEARCH);
        if (query == null || qstr == null || simpleSearchStr == null) {
            return;
        }
        
        SimpleSearch simpleSearch = SimpleSearch.toSimpleSearch(simpleSearchStr);
        ElevateServices elevateServices = ConstellioSpringUtils.getElevateServices();
        String elevateQueryId = elevateServices.toElevateQueryId(simpleSearch);
        
        // Adjusted for Query Filters
        String modifiedParam = params.get(ConstellioSolrQueryParams.LUCENE_QUERY);
        // quelqu un est passé par solr pour effectuer une recherche ex. SolrServices.get
...
//        qstr = getAnalyzedQuery(qstr);
        ...
        try {
            Map<String, ElevationObj> elevationMap = getElevationMap(reader, req.getCore());
//            booster = elevationMap.get(qstr);
            booster = elevationMap.get(elevateQueryId);
            if (booster == null) {
              booster = new ElevationObj(collectionName, elevateQueryId, new ArrayList<String>(),
              new ArrayList<String>());
              elevationMap.put(elevateQueryId, booster);
//                booster = new ElevationObj(collectionName, qstr, new ArrayList<String>(),
//                    new ArrayList<String>());
//                elevationMap.put(qstr, booster);
            }
        } catch (Exception ex) {
            throw new SolrException(SolrException.ErrorCode.SERVER_ERROR, "Error loading elevation", ex);
        }

        ElevationInclude boosterInclude = booster != null ? booster.getElevationInclude() : null;
        if (booster != null) {
            // Change the query to insert forced documents
            BooleanQuery newq = new BooleanQuery(true);

            newq.add(query, BooleanClause.Occur.SHOULD);
            newq.add(boosterInclude.include, BooleanClause.Occur.SHOULD);
            if (booster.exclude != null) {
                for (BooleanClause bq : booster.exclude) {
                    newq.add(bq);
                }
            }
            List<Record> pendingExclusions = recordServices.listMarkedForExclusionOrDeletion(collection,
                Integer.MAX_VALUE);
            for (Record pendingExclusion : pendingExclusions) {
                TermQuery tq = new TermQuery(new Term(idField, pendingExclusion.getUrl()));
                newq.add(new BooleanClause(tq, BooleanClause.Occur.MUST_NOT));
            }
            if (collection.isFederationOwner()) {
                List<FederationRecordDeletionRequired> exclusionOrDeletionRequiredList = 
                	federationServices.listMarkedForExclusionOrDeletion(collection, Integer.MAX_VALUE);
                for (FederationRecordDeletionRequired federationRecordDeletionRequired : exclusionOrDeletionRequiredList) {
					String recordUrl = federationRecordDeletionRequired.getRecordUrl();
	                TermQuery tq = new TermQuery(new Term(idField, recordUrl));
	                newq.add(new BooleanClause(tq, BooleanClause.Occur.MUST_NOT));
				}
            }
            rb.setQuery(newq);
            ...
            if (sortSpec.getSort() == null) {
                sortSpec.setSort(new Sort(new SortField[] {
                    new SortField(idField, boosterInclude.comparatorSource, false),
                ...
                if (force && current[0].getType() != SortField.SCORE) {
                    sorts.add(new SortField(idField, boosterInclude.comparatorSource, false));
                    modify = true;
                }
                for (SortField sf : current) {
                    if (sf.getType() == SortField.SCORE) {
                        sorts.add(new SortField(idField, boosterInclude.comparatorSource, sf.getReverse()));
                        modify = true;
                    }
                    sorts.add(sf);
                }
        ...
        if (rb.isDebug()) {
            ...
            if (booster != null) {
                ...
                match = new ArrayList<String>(boosterInclude.priority.size());
                for (Object o : boosterInclude.include.clauses()) {
    ...
    public String getSourceId() {
        return "$Id: ConstellioElevationComponent.java 1 2010-10-13 23:58:00Z vdussault $";
    }

    @Override
    public String getSource() {
        return "$URL: http://svn.doculibre.com/constellio/trunk/source/java/com/doculibre/constellio/solr/handler/component/ConstellioElevationComponent.java $";
    }
    ...
    public FieldComparator newComparator(final String fieldname, final int numHits, int sortPos,
        ...
        return new FieldComparator() {
            ...
            public int compare(int slot1, int slot2) {
                return values[slot2] - values[slot1]; // values will be small enough that there is no overflow
                // concern
            }
