...
public class ElevateServicesImpl implements ElevateServices {
	...
	@Override
	public synchronized void exclude(Record record, RecordCollection collection) {
//		readCache(collectionName);
//
...
//		}
//		
//		if (!queryExclusionCache.contains(docId)) {
//			queryExclusionCache.add(docId);
...
//		}
	...
	private void reloadCore(String collectionName) {
		...
		SearchHandler searchHandler = (SearchHandler) solrCore.getRequestHandler("/elevate");
		List<SearchComponent> searchComponents = searchHandler.getComponents();
	...
	private synchronized void readCache(String collectionName) {
		...
		if (solrCoreElevationCache == null) {
			...
			File elevateFile = getElevateFile(collectionName);

			SAXReader reader = new SAXReader();
			Document document;
			try {
				document = reader.read(elevateFile);
	...
	@SuppressWarnings("unchecked")
	private synchronized void replaceQueryElement(String queryText,
		...
		try {
			XMLWriter writer = new XMLWriter(new FileOutputStream(elevateFile), OutputFormat
					.createPrettyPrint());
	...
	private static File getElevateFile(String collectionName) {
		CoreContainer coreContainer = SolrCoreContext.getCores();
		SolrCore solrCore = coreContainer.getCore(collectionName);
		SolrConfig solrConfig = solrCore.getSolrConfig();
		String dataDir = solrConfig.getResourceLoader().getDataDir();
		return new File(dataDir, "elevate.xml");
	}
