...
public class AutocompleteServicesImpl implements AutocompleteServices {
    ...
    // N'est pas ajouté aux indexfield entities donc ne sera jamais affiché à l'utilisateur
    ...
    public NamedList<Object> suggest(String q, IndexField indexField) {
        ...
        // FIXME valider avec Vincent si on se base sur le champ de copie (dans le cas des chaines de
        // caractere)?
    ...
    static private NamedList<Object> suggest(String q, String fieldName, SolrServer server,
         ...
         * query.setParam(TermsParams.TERMS_LOWER, q);
         * // * Set terms.prefix to the input term
         * query.setParam(TermsParams.TERMS_PREFIX, q);
         * // * Set terms.lower.incl to false
         * query.setParam(TermsParams.TERMS_LOWER_INCLUSIVE, "false");
        ...
        query.setQueryType(SolrServices.AUTOCOMPLETE_QUERY_NAME);

        try {
            QueryResponse qr = server.query(query);
            NamedList<Object> values = qr.getResponse();
            NamedList<Object> terms = (NamedList<Object>) values.get("terms");// TermsResponse resp =
                                                                              // qr.getTermsResponse();
            ...
            for (int i = 0; i < suggestions.size(); i++) {
                ...
                if (isStringField) {
                    if (currentSuggestion.contains(q)) {
                        // String suffix = StringUtils.substringAfter(currentSuggestion, q);
                        // if (suffix.isEmpty() && !currentSuggestion.equals(q)){
                        // //q n est pas dans currentSuggestion
                        // break;
                        // }
                        if (currentSuggestion.contains(SPECIAL_CHAR)) {
                            // le resultat de recherche retourne des fois une partie de la valeur existant
                            // dans le champ!
     ...
     * //supprimer le texte analysé de la réponse
     * for (String currentPhrase : suggestedList){
     * String originalPhrase = StringUtils.substringBefore(currentPhrase, SPECIAL_CHAR);
     * returnList.add(originalPhrase);
     * }
     * return returnList;
     * }
     */
    ...
    /**
     * Creates a new field associated to the given one. The created copy field will contain the analyzed text
     * followed by the original one.
     * Both texts are separated by SPECIAL_CHAR
     * 
     * @param indexField
    ...
    public void onDocumentAddToAutoCompleteField(SolrInputDocument doc, IndexField indexField, Record record) {
        ...
        for (Object fieldValue : fieldValues) {
            ...
            // FIXME Valider avec Vincent que l'on se base sur le champ source (cas des String)
    ...
    private IndexField getAssociatedIndexField(IndexField indexField) {
        ...
        /*
         * IndexFieldServices indexFieldServices = ConstellioSpringUtils.getIndexFieldServices();
         * String name = getAssociatedIndexFieldName(indexField);
         * RecordCollection collection = indexField.getRecordCollection();
         * IndexField associatedIndexField = indexFieldServices.get(name, collection);
         * return associatedIndexField;
         */
    }
...
//    }
    ...
    private boolean isValidAutocompleteSuggestion(
    		String input, 
    		String analyzedInput,
    		String suggestion, 
    		String analyzedSuggestion,
    		RecordCollection collection) {
    	boolean valid = false;
    	readBlacklistCache();
    	if (!blacklistCache.contains(suggestion)) {
	    	...
	    	try {
		    	...
		    	if (thesaurus != null) {
		        	...
		        	for (SkosConcept thesaurusSuggestion : prefLabelSuggestions) {
						...
						if (isValidAutocompleteSuggestion(input, analyzedInput, prefLabel, analyzedSuggestion, collection) && 
		        	...
		        	if (maxResults > 0) {
			        	...
			        	for (SkosConcept thesaurusSuggestion : altLabelSuggestions) {
			        		for (String altLabel : thesaurusSuggestion.getAltLabels(locale)) {
								...
								if (isValidAutocompleteSuggestion(input, analyzedInput, altLabel, analyzedSuggestion, collection) && 
										!analyzedSuggestions.contains(analyzedSuggestion)) {
									maxResults--;
		    	...
		    	if (maxResults > 0) {
		        	...
		        	for (String mostPopularQueriesSuggestion : mostPopularQueriesSuggestions) {
						...
						if (isValidAutocompleteSuggestion(input, analyzedInput, mostPopularQueriesSuggestion, analyzedSuggestion, collection) && 
								!analyzedSuggestions.contains(analyzedSuggestion)) {
							maxResults--;
	...
	public void blacklistAutocomplete(String term) {
		readBlacklistCache();
		if (!blacklistCache.contains(term)) {
			blacklistCache.add(term);
			writeBlacklistFile();
		}
	}
	...
	public List<String> getBlacklistedAutocompleteTerms() {
		readBlacklistCache();
		return new ArrayList<String>(blacklistCache);
	}
	...
	public void cancelBlacklistedAutocomplete(String term) {
		readBlacklistCache();
		if (blacklistCache.contains(term)) {
			blacklistCache.remove(term);
			writeBlacklistFile();
		}
	}
	
	private synchronized void readBlacklistCache() {
		if (blacklistCache == null) {
			blacklistCache = new ArrayList<String>();
			File blackListFile = getBlacklistFile();
			if (blackListFile.exists()) {
				try {
					blacklistCache.addAll(FileUtils.readLines(blackListFile));
					Collections.sort(blacklistCache);
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
			}
	...
	private synchronized void writeBlacklistFile() {
		File blackListFile = getBlacklistFile();
		try {
			FileUtils.writeLines(blackListFile, blacklistCache, false);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
	...
	private static File getBlacklistFile() {
		...
		return new File(logsRootDir, "autocomplete-blacklist.txt");
	}
    ...
    public static void main(String[] args) {
        ...
        SolrCoreContext.init(new File(path));
        ...
        /*
         * print ("t", suggest(AnalyzerUtils.analyzePhrase("t"), "constellio_mail_mainFolder_analyzedCopy",
         * testServer, true));
         * print ("té", suggest(AnalyzerUtils.analyzePhrase("té", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("te", suggest(AnalyzerUtils.analyzePhrase("te", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("tes", suggest(AnalyzerUtils.analyzePhrase("tes", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("test", suggest(AnalyzerUtils.analyzePhrase("test", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("Test", suggest(AnalyzerUtils.analyzePhrase("Test", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("Të", suggest(AnalyzerUtils.analyzePhrase("Të", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("B", suggest(AnalyzerUtils.analyzePhrase("B", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("Bo", suggest(AnalyzerUtils.analyzePhrase("Bo", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * print ("b", suggest(AnalyzerUtils.analyzePhrase("b", false),
         * "constellio_mail_mainFolder_analyzedCopy", testServer, true));
         * System.out.println("Autocomplete avec champ de type text_fr:");
         * print ("B", suggest("B", "doc_title", testServer, false));
         * print ("Boost", suggest("Boost", "doc_title", testServer, false));
         * print ("Böost", suggest("Böost", "doc_title", testServer, false));
         * print ("g", suggest("g", "doc_title", testServer, false));
         */
