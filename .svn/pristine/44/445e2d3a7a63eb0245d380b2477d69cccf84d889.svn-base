...
public class SolrServletEmulator {
	...
	public void writeResponse(String solrQuery, RecordCollection collection, ConstellioUser user, HttpServletRequest request, HttpServletResponse response) {
		...
		if (user != null && !user.hasSearchPermission(collection)) {
			...
			ModifiableSolrParams solrParams = new ModifiableSolrParams(SolrRequestParsers.parseQueryString(solrQuery));

			String queryType = solrParams.get("qt");
			if (queryType != null && (queryType.toLowerCase().equals("spellchecker") || queryType.toLowerCase().equals("spellcheck"))) {
	...
	private String addConstellioFacets(SimpleSearch simpleSearch, String solrParams, RecordCollection collection, ConstellioUser user) {
		...
		String fakeQueryWithFacets = FacetServicesImpl.toSolrQuery(simpleSearch, 0, 100, true, false, null, null, user).toString();
	...
	private void writeSpellCheckerResponse(SimpleSearch simpleSearch, SolrParams solrParams, RecordCollection collection, ConstellioUser user, OutputStream outputStream) {
		...
		NamedList<Object> responseHeader = (NamedList<Object>) successfulNamedList.get(ServletsConstants.RESPONSE_HEADER);
